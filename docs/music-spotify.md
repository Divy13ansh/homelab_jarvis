...
- **Competing Spotify plugin:** an unofficial `@shadowninex/openclaw-spotify` plugin may get auto-installed if `plugins.entries.spotify` ever appears in config. Its native tools are unauthenticated and its skill shadows ours. We do NOT use it — remove with `node dist/index.js plugins uninstall spotify --force`, delete `~/.openclaw/plugin-skills/spotify`, restart gateway. Single path: skill + `spotify_adapter.py`.
- **Bot refuses despite working adapter:** session-history anchoring — old refusals in the main session (shared with Discord DMs) pattern-continue. Fix: run 2–3 explicit successful adapter turns (`devices`, `transfer`, `play`, `status`) to bank clean history, then retest the plain prompt. `agent:main:main` cannot be deleted; `sessions compact --max-lines N` truncates as last resort.
- **API ok but no sound:** `play` returns 204 while `status` shows `is_playing:false`, `item:null` means the desktop app is asleep/backgrounded. Open Spotify on the device and press play once; API control takes over after that.
